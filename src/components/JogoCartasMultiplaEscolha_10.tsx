...
// Definição das cartas com categorias
const cartas = [
  {
...
const TelaInicial = ({ onStartGame, onReset, categoriasDisponiveis, onCategoriaChange }) => {
  ...
  const marcarTodasCategorias = () => {
    setSelectedCategorias(categoriasDisponiveis);
  };
  ...
  return (
    <Card className="w-full max-w-sm mx-auto mt-8">
      ...
      <CardContent>
        ...
        <div className="mb-4">
          <h3 className="font-bold">Selecione Categorias:</h3>
          ...
          <div className="flex mt-2">
            <Button onClick={marcarTodasCategorias} variant="outline" className="flex-1 mr-1">
              Marcar Todas
            </Button>
            <Button onClick={limparCategorias} variant="outline" className="flex-1 ml-1">
              Limpar Tudo
            </Button>
          </div>
        </div>
      ...
      <CardFooter className="flex justify-between">
        <Button
          onClick={() => onStartGame(selectedCategorias)}
...
const EcoChallenge = () => {
  ...
  useEffect(() => {
    if (jogoIniciado && cartasFiltradas.length > 0) {
      selecionarCartaAleatoria();
    }
  }, [jogoIniciado, cartasFiltradas]);
  ...
  const iniciarJogoComCategorias = (categoriasSelecionadas) => {
  ...
  if (!jogoIniciado) {
    return (
      <TelaInicial
        onStartGame={iniciarJogoComCategorias}
        onReset={resetarJogo}
