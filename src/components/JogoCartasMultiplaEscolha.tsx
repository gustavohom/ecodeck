...
import { ScrollArea } from "@/components/ui/scroll-area"; // Garantir que ScrollArea está importado
...
function parseJSDeckFile(content: string): Carta[] {
    ...
    } catch (error: any) { console.error("Erro ao processar arquivo JS:", error); throw new Error(`Erro ao processar arquivo JS: ${error.message}`); }
}
...
const TelaInicial: React.FC<TelaInicialProps> = ({
    ...
    const [playerInputs, setPlayerInputs] = useState<PlayerInput[]>(() => initialPlayers.length > 0 ? initialPlayers.map((p) => ({ id: p.id, name: p.name, color: p.color, showColorPicker: false })) : [{ id: Date.now(), name: "", color: predefinedColors[0], showColorPicker: false }] ); // Use Date.now() for initial unique ID
    ...
    useEffect(() => {
        setIsClient(true); const savedCustomRaw = localStorage.getItem("customSourceInfos"); let loadedCustomSources: SourceInfo[] = [];
        try { const parsedCustom = savedCustomRaw ? JSON.parse(savedCustomRaw) : []; loadedCustomSources = Array.isArray(parsedCustom) ? parsedCustom.map(s => ({...s, cards: Array.isArray(s.cards) ? s.cards : []})) : []; } catch (e) { console.error("Erro ao carregar customSourceInfos:", e); loadedCustomSources = []; }
        ...
        const savedBuiltInStatusRaw = localStorage.getItem("builtInSourceStatus"); let loadedBuiltInStatus: Record<string, boolean> | null = null; try { loadedBuiltInStatus = savedBuiltInStatusRaw ? JSON.parse(savedBuiltInStatusRaw) : null; } catch (e) { console.error("Erro ao carregar builtInSourceStatus:", e); loadedBuiltInStatus = null; }
    ...
    const handleCustomDeckUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                let loadedCards: Carta[] = []; const sourceName = file.name.replace(/\.(js|json)$/, ""); const sourceId = `custom_${Date.now()}_${i}_${Math.random().toString(16).slice(2)}`;
                if (allSources.some(s => s.name === sourceName && s.type === 'custom')) { errors.push(`Fonte "${sourceName}" já carregada.`); continue; }
                if (file.name.endsWith(".js")) { loadedCards = parseJSDeckFile(content); }
                else if (file.name.endsWith(".json")) { const raw = JSON.parse(content) as any[]; if (!Array.isArray(raw)) throw new Error("JSON não é um array."); loadedCards = raw.map((card, index) => ({ ...card, id: card.id || `${sourceId}_card_${index}_${Math.random().toString(16).slice(2)}` })) as Carta[]; }
                else { errors.push(`Formato não suportado: ${file.name}.`); continue; }
                if (loadedCards.length === 0) { errors.push(`Nenhuma carta encontrada em "${sourceName}".`); continue; }
                const { processedCards, internalBaralhos, totalCards } = processCardsAndExtractBaralhos(loadedCards);
                newCustomSources.push({ id: sourceId, name: sourceName, type: 'custom', cards: processedCards, internalBaralhos, totalCards, active: true });
            } catch (error: any) { errors.push(`Erro ao ler ${file.name}: ${error.message}`); }
        }
        if (newCustomSources.length > 0) { setAllSources(prev => [...prev, ...newCustomSources]); setActiveInternalBaralhos(prev => { const newState = { ...prev }; newCustomSources.forEach(source => { newState[source.id] = new Set(Object.keys(source.internalBaralhos)); }); return newState; }); setExpandedSources(prev => { const newSet = new Set(prev); newCustomSources.forEach(s => newSet.add(s.id)); return newSet; }); }
    ...
    const handleStartGame = (continueGame = false) => {
        ...
        if (playerInputs.filter(p => p.name.trim()).length === 0) { alert("Adicione e nomeie pelo menos um jogador."); return; }
        const activeSources = allSources.filter(s => s.active); if (activeSources.length === 0) { alert("Nenhuma Fonte de Cartas ativa."); return; }
        const anyInternalDeckActive = activeSources.some(source => activeInternalBaralhos[source.id]?.size > 0); if (!anyInternalDeckActive) { alert("Nenhum baralho interno ativo nas fontes selecionadas."); return; }
        const { categorias: finalCategoriasDisponiveis } = recalcularCategoriasAtivas(allSources, activeInternalBaralhos);
        const finalCategoriasSelecionadas = categoriasSelecionadas.filter(cat => finalCategoriasDisponiveis.includes(cat)); if (finalCategoriasSelecionadas.length === 0) { alert("Nenhuma categoria selecionada ou disponível com os baralhos ativos."); return; }
        const finalActiveCards = activeSources.flatMap(source => { const activeBaralhosSet = activeInternalBaralhos[source.id]; if (!activeBaralhosSet || activeBaralhosSet.size === 0) return []; return source.cards.filter(card => activeBaralhosSet.has(card.baralho || DEFAULT_BARALHO_NAME) && card.categorias?.some(cat => finalCategoriasSelecionadas.includes(cat))); }); if (finalActiveCards.length === 0) { alert("Nenhuma carta encontrada com a combinação de baralhos e categorias."); return; }
    ...
    return (
        <Card className="w-full max-w-lg mx-auto mt-8 mb-8 shadow-lg">
            ...
            <CardContent className="space-y-6">
                {/* Gerenciamento de Fontes e Baralhos */}
                ...
                <div className="space-y-3">
                    ...
                    {/* CORREÇÃO: ScrollArea adicionada aqui */}
                    <ScrollArea className="max-h-56 space-y-2 pr-2 border rounded-md bg-gray-50 p-2">
                        {playerInputs.map((player, index) => (
                            <div key={player.id} className="border p-3 rounded-md shadow-sm bg-white relative"> {/* Removido mb-2, space-y na ScrollArea */}
                                <div className="flex items-center space-x-2">
...
const EcoChallenge: React.FC = () => {
    ...
    const [mostrarSomentePerguntas, setMostrarSomentePerguntas] = useState(false); // Mantido, embora não usado na lógica de filtro
        ...
        if (initialGameState && initialGameState.jogoIniciado && sourcesForGame && sourcesForGame.length > 0) {
            ...
            setCartaAtual(null); setNoCardsAvailable(false); setRespondido(false); setMensagem(""); setMostrarDica(false); setDicaUsada(false); setMostrarFontes(false); setOpcoesEliminadas([]); setSelecionado(null); setSelecoesMultiplas([]); setOrdemSelecoes([]); setTempoRestante(null); setSelecaoColunaA(null); setParesFormados([]); setCoordenadasClique(null); setFragmentosSelecionados([]); setRolledNumber(null); setIsDieModalOpen(false); setCartaRevelada(!(initialGameState.ocultarCarta ?? true));
             ...
             if (typeof window !== "undefined") {
                const customSourcesRaw = localStorage.getItem("customSourceInfos"); const builtInStatusRaw = localStorage.getItem("builtInSourceStatus");
                let customSources: SourceInfo[] = []; let builtInStatus: Record<string, boolean> | null = null;
                try { customSources = customSourcesRaw ? JSON.parse(customSourcesRaw) : []; } catch { customSources = []; } try { builtInStatus = builtInStatusRaw ? JSON.parse(builtInStatusRaw) : null; } catch { builtInStatus = null; }
                const processedCustomSources = customSources.map(source => { const { processedCards, internalBaralhos, totalCards } = processCardsAndExtractBaralhos(source.cards || []); return { ...source, cards: processedCards, internalBaralhos, totalCards, type: 'custom' as const }; });
                ...
                if (reconstructedSources.length > 0) {
                    console.log("Fontes reconstruídas:", reconstructedSources); setGameState(initialGameState as GameState); setCurrentGameSources(reconstructedSources);
                    setCartaAtual(null); setNoCardsAvailable(false); setRespondido(false); setMensagem(""); setMostrarDica(false); setDicaUsada(false); setMostrarFontes(false); setOpcoesEliminadas([]); setSelecionado(null); setSelecoesMultiplas([]); setOrdemSelecoes([]); setTempoRestante(null); setSelecaoColunaA(null); setParesFormados([]); setCoordenadasClique(null); setFragmentosSelecionados([]); setRolledNumber(null); setIsDieModalOpen(false); setCartaRevelada(!(initialGameState.ocultarCarta ?? true));
                } else { console.error("Falha ao reconstruir fontes."); setGameState(null); setCurrentGameSources([]); }
             } else { console.error("Não é possível reconstruir fontes."); setGameState(null); setCurrentGameSources([]); }
        } else { console.error("handleGameStart sem dados válidos."); setGameState(null); setCurrentGameSources([]); }
     }, []);
    const updateGameState = useCallback((newState: Partial<GameState>) => {
        setGameState(prev => { if (!prev) return null; const updatedState = { ...prev, ...newState }; if (typeof window !== "undefined") { try { localStorage.setItem("estadoEcoChallenge", JSON.stringify(updatedState)); } catch (e) { console.error("Erro ao salvar estado:", e); } } return updatedState; });
    }, []);
    ...
    const selecionarCartaAleatoria = useCallback(() => {
        ...
        const cartasFiltradas = currentGameSources.flatMap(source => { const activeBaralhosArray = activeInternalBaralhosState[source.id]; if (!activeBaralhosArray || activeBaralhosArray.length === 0) return []; const activeBaralhoSet = new Set(activeBaralhosArray);
            return source.cards.filter(card => {
                ...
                if (mostrarSomentePerguntas && !tiposPergunta.includes(card.tipo)) return false; // Mantido filtro opcional
                return true;
            });
        });
    ...
    useEffect(() => { if (gameState?.jogoIniciado && currentGameSources.length > 0 && !cartaAtual && !noCardsAvailable) { console.log("Selecionando primeira carta..."); selecionarCartaAleatoria(); } }, [gameState?.jogoIniciado, currentGameSources, cartaAtual, noCardsAvailable, selecionarCartaAleatoria]); // Adicionado selecionarCartaAleatoria de volta aqui, monitorando se causa loop
    useEffect(() => {
                ...
                setTempoRestante((prev) => {
                    if (prev === null || prev <= 1) {
                        ...
                        setGameState(currentGameState => {
                           ...
                           const finalState = { ...currentGameState, players: updatedPlayers }; if (typeof window !== "undefined") { localStorage.setItem("estadoEcoChallenge", JSON.stringify(finalState)); } return finalState;
                       }); return 0;
                    } return prev - 1;
                });
            }, 1000);
    ...
    const verificarResposta = () => {
        if (!cartaAtual || !gameState || !gameState.players || gameState.currentPlayerId === null || respondido) { console.warn("verificarResposta inválido."); return; }
        const currentPlayer = gameState.players.find(p => p.id === gameState.currentPlayerId); if (!currentPlayer) { console.error("Jogador atual não encontrado."); return; }
        console.log("Verificando resposta:", cartaAtual.tipo); let cor = false; let pontosGanhos = 20; let pontosPerdidos = 10; let darPuloDificil = false; let mensagemFinal = ""; let aplicarEfeitoPadrao = false;
        ...
        setMensagem(mensagemFinal.trim()); console.log("Resultado:", cor ? "Correto" : "Incorreto", "| Msg:", mensagemFinal);
    };
    ...
    const voltarTelaInicial = () => { if (window.confirm("Voltar para Tela Inicial? Progresso salvo.")) { setGameState(null); setCartaAtual(null); setNoCardsAvailable(false); setRespondido(false); setMensagem(""); setCurrentGameSources([]); } };
    ...
    // CORREÇÃO: Remover Math.max para permitir contador negativo
            ...
            >
                ...
                <CardFooter className="flex flex-col items-center pt-4 border-t bg-gray-50/50 rounded-b-lg">
                    ...
                    <div className="flex flex-wrap justify-center gap-1.5 w-full mb-3 p-2 border rounded-md bg-gray-100 shadow-inner">
                        <Button onClick={diminuirAcertos} variant="outline" size="icon" className="h-8 w-8" title="Diminuir Acertos (-1)"><ThumbsUp className="h-4 w-4 text-green-600 transform scale-x-[-1]" /></Button>
                        <Button onClick={diminuirErros} variant="outline" size="icon" className="h-8 w-8" title="Diminuir Erros (-1)"><ThumbsDown className="h-4 w-4 text-red-600 transform scale-x-[-1]" /></Button>
                        {/* CORREÇÃO: Cor da estrela para remover */}
                        <Button onClick={diminuirContadorDeEstrelas} variant="outline" size="icon" className="h-8 w-8" title="Diminuir Estrela Bônus (-1)">
                            <Star className="h-4 w-4 text-red-500" /> {/* Estrela vermelha */}
                        </Button>
                        <Button onClick={incrementarContadorDeEstrelas} variant="outline" size="icon" className="h-8 w-8" title="Aumentar Estrela Bônus (+1)"><Star className="h-4 w-4 text-yellow-500 fill-current" /></Button>
                        <Button onClick={diminuirRodadasPreso} variant="outline" size="icon" className="h-8 w-8" title="Diminuir Rodada Preso (-1)"><ChevronDown className="h-5 w-5 text-purple-600" /></Button>
                        <Button onClick={incrementarRodadasPreso} variant="outline" size="icon" className="h-8 w-8" title="Aumentar Rodada Preso (+1)"><ChevronUp className="h-5 w-5 text-purple-600" /></Button>
                    </div>
                    {/* Botão Principal */}
                    <div className="w-full mb-3">
                        {isCartaOculta && !cartaRevelada ? (
                            ...
                            <Button
                                ...
                                // CORREÇÃO: handlers de long press mantidos para permitir rolar dado
            ...
            {isDieModalOpen && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50 backdrop-blur-sm">
                    <div className="bg-white p-6 rounded-lg shadow-xl text-center relative w-64 h-64 flex flex-col justify-center items-center border-4 border-gray-300">
                        <Button className="absolute top-2 right-2 text-gray-500 hover:text-gray-800" variant="ghost" size="icon" onClick={() => setIsDieModalOpen(false)} disabled={isRolling} aria-label="Fechar dado"><XIcon className="h-6 w-6" /></Button>
                        {isRolling ? ( <> <p className="text-lg mb-4 font-semibold text-gray-700">Rolando...</p><p className="text-7xl font-bold mb-6 text-blue-600 animate-bounce">{rollingNumber}</p><div className="h-10"></div> </>
                        ) : ( <> <p className="text-lg mb-2 font-semibold text-gray-700">Resultado:</p><p className="text-8xl font-bold mb-4 text-green-700">{rolledNumber}</p><Button onClick={rolarDado} size="lg" variant="secondary" className="mt-2"><Dice6 className="h-5 w-5 mr-2" /> Rolar Novamente</Button> </> )}
                    </div>
                </div>
            )}
        </div>
