...
interface SourceInfo {
    ...
    internalBaralhos: Record<string, number>; active: boolean;
}
...
function processCardsAndExtractBaralhos(cards: Carta[]): { processedCards: Carta[], internalBaralhos: Record<string, number> } {
    ...
    const processedCards = cards.map(card => { const baralhoName = card.baralho?.trim() || DEFAULT_BARALHO_NAME; baralhoCounts[baralhoName] = (baralhoCounts[baralhoName] || 0) + 1; return { ...card, baralho: baralhoName }; });
    const sortedBaralhoNames = Object.keys(baralhoCounts).sort(); const sortedInternalBaralhos: Record<string, number> = {}; sortedBaralhoNames.forEach(name => { sortedInternalBaralhos[name] = baralhoCounts[name]; }); return { processedCards, internalBaralhos: sortedInternalBaralhos };
}
const builtInSourcesData: Omit<SourceInfo, 'active' | 'internalBaralhos'>[] = [ { id: "manejoPlantadas", name: "Manejo Plantadas", type: 'builtin', cards: manejoPlantadas as Carta[] }, { id: "manejoNativas", name: "Manejo Nativas", type: 'builtin', cards: manejoNativas as Carta[] }, { id: "ecologiaFlorestal", name: "Ecologia Florestal", type: 'builtin', cards: ecologiaFlorestal as Carta[] }, { id: "estrelasAliens", name: "Estrelas & Aliens (DLC)", type: 'builtin', cards: estrelasAliens as Carta[] }, { id: "testCards", name: "Test Cards", type: 'builtin', cards: testCards as Carta[] }, ];
const initialBuiltInSources: SourceInfo[] = builtInSourcesData.map(source => { const { processedCards, internalBaralhos } = processCardsAndExtractBaralhos(source.cards); return { ...source, cards: processedCards, internalBaralhos: internalBaralhos, active: true }; });
...
function recalcularCategoriasAtivas(allSources: SourceInfo[], activeInternalBaralhos: Record<string, Set<string>>): string[] { const activeCards: Carta[] = []; allSources.forEach(source => { if (!source.active) return; const activeBaralhosForSource = activeInternalBaralhos[source.id]; if (!activeBaralhosForSource) return; source.cards.forEach(card => { if (activeBaralhosForSource.has(card.baralho || DEFAULT_BARALHO_NAME)) { activeCards.push(card); } }); }); return Array.from(new Set(activeCards.flatMap(c => c.categorias || []))).sort(); }
...
const TelaInicial: React.FC<TelaInicialProps> = ({
    ...
    const [allSources, setAllSources] = useState<SourceInfo[]>(() => {
         if (typeof window !== "undefined") {
             const savedCustom = localStorage.getItem("customSourceInfos"); const savedBuiltInStatus = localStorage.getItem("builtInSourceStatus"); let customSources: SourceInfo[] = []; let builtInStatus: Record<string, boolean> | null = null;
             try { customSources = savedCustom ? JSON.parse(savedCustom) : []; } catch { customSources = []; } try { builtInStatus = savedBuiltInStatus ? JSON.parse(savedBuiltInStatus) : null; } catch { builtInStatus = null; }
             const processedCustomSources = customSources.map(source => { const { processedCards, internalBaralhos } = processCardsAndExtractBaralhos(source.cards); return { ...source, cards: processedCards, internalBaralhos }; });
             const combined = [...initialBuiltInSources.map(bs => ({...bs, active: builtInStatus ? (builtInStatus[bs.id] ?? true) : true})), ...processedCustomSources]; return combined;
    ...
    const [activeInternalBaralhos, setActiveInternalBaralhos] = useState<Record<string, Set<string>>>(() => {
        let initialState: Record<string, Set<string>> = {};
         if (typeof window !== "undefined") {
             const saved = localStorage.getItem("activeInternalBaralhos"); try { const parsed = saved ? JSON.parse(saved) : {}; Object.keys(parsed).forEach(key => { if (Array.isArray(parsed[key])) { initialState[key] = new Set(parsed[key]); } }); } catch { initialState = {}; }
         }
         // Itera sobre as fontes que acabaram de ser inicializadas no estado 'allSources'
         allSources.forEach(source => { // Usa a variável 'allSources' do escopo do useState
             if (!initialState[source.id]) { initialState[source.id] = new Set(Object.keys(source.internalBaralhos)); }
             else { Object.keys(source.internalBaralhos).forEach(bName => { initialState[source.id].add(bName); }); initialState[source.id].forEach(savedBName => { if (!(savedBName in source.internalBaralhos)) { initialState[source.id].delete(savedBName); }}); }
         });
         return initialState;
    });
        ...
        if (continueGame && typeof window !== "undefined") {
            ...
            try {
                ...
                if (savedState && savedState.jogoIniciado) {
                    gameStateToPass = { ...savedState, categoriasSelecionadas: finalCategoriasSelecionadas, ocultarCarta: ocultarCarta, probabilityIndex: probabilityIndex, };
            ...
            gameStateToPass = { players: initializedPlayers, currentPlayerId: initializedPlayers[0]?.id ?? null, categoriasSelecionadas: finalCategoriasSelecionadas, ocultarCarta: ocultarCarta, probabilityIndex: probabilityIndex, jogoIniciado: true, };
        }

        // Passa o estado do jogo E as fontes ativas separadamente
        onStartGame(gameStateToPass, activeSourcesForGame); // AQUI: Passando activeSourcesForGame
    };
...
const EcoChallenge: React.FC = () => {
    ...
    const [currentGameSources, setCurrentGameSources] = useState<SourceInfo[]>([]); // Fontes para o jogo atual
    ...
    // Callback para iniciar/continuar o jogo, recebendo o estado e as fontes da TelaInicial
    const handleGameStart = useCallback((initialGameState?: Partial<GameState>, sourcesForGame?: SourceInfo[]) => {
        if (initialGameState && sourcesForGame) {
            setGameState(initialGameState as GameState);
            setCurrentGameSources(sourcesForGame);
            // Resetar estados da rodada ao iniciar/continuar
            setCartaAtual(null); // Força a seleção de uma nova carta pelo useEffect
            setNoCardsAvailable(false);
            setRespondido(false);
            setMensagem("");
            setMostrarDica(false);
            setDicaUsada(false);
            // ... resetar outros estados de interação de carta ...
            setSelecionado(null); setSelecoesMultiplas([]); setOrdemSelecoes([]); /* ... etc */
        } else if (initialGameState) { // Tenta reconstruir fontes se não foram passadas (caso de save antigo)
            console.warn("Reconstruindo fontes para jogo salvo...");
            ...
            setCurrentGameSources(reconstructedSources);
             ...
             setCartaAtual(null); setNoCardsAvailable(false); setRespondido(false); setMensagem(""); setMostrarDica(false); setDicaUsada(false); setSelecionado(null); setSelecoesMultiplas([]); setOrdemSelecoes([]);
        }
    }, []); // Callback definido uma vez
    ...
    const updateGameState = useCallback((newState: Partial<GameState>) => {
        setGameState(prev => {
            ...
            if (typeof window !== "undefined") {
                try {
                    // Salva o estado principal do jogo
                    localStorage.setItem("estadoEcoChallenge", JSON.stringify(updatedState));
                    // A persistência das fontes/baralhos ativos é feita na TelaInicial
    ...
    // Carregar estado salvo ao montar (se não iniciado pela TelaInicial)
    useEffect(() => {
        if (!gameState && typeof window !== "undefined") {
            const savedStateRaw = localStorage.getItem("estadoEcoChallenge");
             // Tenta carregar, mas não define as fontes aqui, espera handleGameStart
            try {
                const savedState = savedStateRaw ? JSON.parse(savedStateRaw) as GameState : null;
                if (savedState && savedState.jogoIniciado) {
                     // Chama handleGameStart para inicializar corretamente, incluindo fontes
                     handleGameStart(savedState);
                }
            } catch (e) {
                console.error("Erro ao carregar estado:", e);
                localStorage.removeItem("estadoEcoChallenge");
                localStorage.removeItem("activeSourceIds"); // Limpa dados relacionados
                localStorage.removeItem("activeInternalBaralhosState");
            }
        }
    }, [handleGameStart, gameState]); // Depende de gameState para não rodar se já iniciado

    // Selecionar carta aleatória
    const selecionarCartaAleatoria = useCallback(() => {
        if (!gameState || currentGameSources.length === 0) {
             console.log("Seleção de carta: GameState ou currentGameSources ausentes/vazios.");
             setNoCardsAvailable(true); // Garante que mostre a mensagem se não houver fontes
             return;
         }
        const { categoriasSelecionadas, probabilityIndex, activeInternalBaralhosState } = gameState; // Usa activeInternalBaralhosState
        ...
        // Usa diretamente currentGameSources (que já são as ativas)
        ...
        console.log("Cartas Filtradas:", cartasFiltradas.length); // Log para depuração
    ...
    }, [gameState, currentGameSources]); // Depende do gameState e das fontes do jogo atual

     // Seleciona a primeira carta quando o jogo inicia E as fontes foram carregadas
     useEffect(() => {
         if (gameState?.jogoIniciado && currentGameSources.length > 0 && !cartaAtual && !noCardsAvailable) {
             console.log("Selecionando primeira carta..."); // Log para depuração
             selecionarCartaAleatoria();
         }
     }, [gameState?.jogoIniciado, currentGameSources, cartaAtual, noCardsAvailable, selecionarCartaAleatoria]);
    ...
    useEffect(() => {
        ...
        } else if (timerIntervalRef.current && (respondido || tempoRestante === 0 || !cartaRevelada)) { clearInterval(timerIntervalRef.current); }
    ...
    const verificarResposta = () => {
        ...
        switch (cartaAtual.tipo) {
            ...
            case "RelacionarColunas": if (!Array.isArray(cartaAtual.respostaCorreta)) { cor = false; break; } cor = paresFormados.length === cartaAtual.respostaCorreta.length && paresFormados.map(p => `${p.aId}-${p.bId}`).sort().join(',') === cartaAtual.respostaCorreta.map(p => `${p.aId}-${p.bId}`).sort().join(','); if (cor) pontosGanhos = 30; darPuloDificil = true; aplicarEfeitoPadrao = true; break;
            ...
            case "Vantagem": if (!Array.isArray(cartaAtual.respostaCorreta)) { cor = false; break; } cor = selecionado !== null && cartaAtual.respostaCorreta.includes(selecionado); mensagemFinal = cor ? (cartaAtual.vantagem || "Vantagem aplicada!") : "Ação não confirmada."; break;
            case "Desvantagem": cor = false; mensagemFinal = cartaAtual.desvantagem || "Desvantagem aplicada."; break;
            case "Outras": if (!Array.isArray(cartaAtual.respostaCorreta)) { cor = false; break; } cor = selecionado !== null && cartaAtual.respostaCorreta.includes(selecionado); mensagemFinal = cor ? (cartaAtual.vantagem || 'Ok!') : (cartaAtual.desvantagem || 'Hmm...'); break;
    ...
    const pularPergunta = () => { const cp = gameState?.players.find(p => p.id === gameState.currentPlayerId); if (!cp || !cartaAtual || respondido || (gameState?.ocultarCarta && !cartaRevelada)) return; if (!tiposPergunta.includes(cartaAtual.tipo)) { setMensagem("Não pode pular este tipo."); return; } if (cp.pulosDisponiveis > 0) { updateCurrentPlayer({ pulosDisponiveis: cp.pulosDisponiveis - 1 }); setMensagem("Carta pulada!"); setTimeout(() => selecionarCartaAleatoria(), 500); } else { setMensagem("Sem pulos disponíveis."); } }; // Passa gameState e currentGameSources
    const eliminarRespostaErrada = () => {
        ...
        if (!tiposEliminaveis.includes(cartaAtual.tipo) || !('opcoes' in cartaAtual) || !Array.isArray(cartaAtual.opcoes) || cartaAtual.opcoes.length <= 2) { setMensagem("Não é possível eliminar opções."); return; }
        ...
        if ('respostaCorreta' in cartaAtual) { if (typeof cartaAtual.respostaCorreta === 'number') { respostaCorretaNumeros = [cartaAtual.respostaCorreta];} else if (Array.isArray(cartaAtual.respostaCorreta) && cartaAtual.respostaCorreta.every(item => typeof item === 'number')) { respostaCorretaNumeros = cartaAtual.respostaCorreta as number[]; } else if (cartaAtual.tipo === 'Vantagem' || cartaAtual.tipo === 'Desvantagem') { if (Array.isArray(cartaAtual.respostaCorreta)) respostaCorretaNumeros = cartaAtual.respostaCorreta as number[]; else respostaCorretaNumeros = cartaAtual.opcoes.map(o => o.id); } else { setMensagem("Erro interno."); return; }
        } else { setMensagem("Erro interno."); return; }
    ...
    const voltarTelaInicial = () => { if (window.confirm("Voltar para a Tela Inicial? Progresso salvo.")) { setGameState(null); setCartaAtual(null); setNoCardsAvailable(false); setRespondido(false); setMensagem(""); setCurrentGameSources([]) } }; // Limpa as fontes do jogo atual
    ...
    if (!gameState) {
        ...
        try { const savedState = savedStateRaw ? JSON.parse(savedStateRaw) as GameState : null; if (savedState && savedState.jogoIniciado) { hasSaved = true; initialPlayersData = savedState.players || []; initialOcultar = savedState.ocultarCarta ?? true; initialProb = savedState.probabilityIndex ?? 0; } } catch {}
        ...
        <div className="flex flex-col items-center p-2 md:p-4 min-h-screen bg-gradient-to-b from-green-50 to-blue-50 font-sans">
             <Card className={cn("w-full max-w-lg mx-auto mt-4 shadow-xl border-2 rounded-lg", obterEstiloCarta())} style={players.length > 0 && currentPlayer && !(ocultarCarta && !cartaRevelada) ? { boxShadow: `0 0 15px 3px ${currentPlayer.color}` } : {}}>
                 <CardHeader className="pb-3">
                     {/* ... Conteúdo do Header (Título, Badge, Timer, ScrollArea Pergunta) ... */}
                       <div className="flex justify-between items-start mb-2 gap-2">
                         <div className="flex items-center space-x-2 flex-1 min-w-0">
                             <div className="flex-1 min-w-0">
                                <CardTitle className="text-lg md:text-xl font-bold leading-tight truncate" title={cartaAtual.titulo}>
                                    {ocultarCarta && !cartaRevelada ? "Carta Oculta" : cartaAtual.titulo}
                                </CardTitle>
                                {(!ocultarCarta || cartaRevelada) && (
                                     <p className="text-xs text-gray-500 mt-1 truncate" title={(cartaAtual.categorias ? cartaAtual.categorias.join(", ") : '') + (cartaAtual.baralho && cartaAtual.baralho !== DEFAULT_BARALHO_NAME ? ` (${cartaAtual.baralho})` : '')}>
                                        {cartaAtual.categorias?.join(", ")}
                                        {cartaAtual.baralho && cartaAtual.baralho !== DEFAULT_BARALHO_NAME && ` (${cartaAtual.baralho})`}
                                    </p>
                                )}
                            </div>
                        </div>
                 ...
                 <CardFooter className="flex flex-col items-center pt-4 border-t bg-gray-50/50 rounded-b-lg">
                    ...
                    <div className="w-full">
                        <Progress value={currentPlayer.progresso} className="h-2.5 [&>*]:bg-orange-500" />
                         <div className="flex justify-between items-center w-full mt-2 text-sm text-gray-700 flex-wrap gap-x-4 gap-y-1 font-semibold">
                             ...
                             <span className="flex items-center" title="Pulos Disponíveis"><SkipForward className="h-4 w-4 text-blue-500 mr-1"/>{currentPlayer.pulosDisponiveis}</span>
                             <span className="flex items-center" title="Respostas Corretas"><ThumbsUp className="h-4 w-4 text-green-500 mr-1"/>{currentPlayer.respostasCertas}</span>
                             <span className="flex items-center" title="Respostas Erradas"><ThumbsDown className="h-4 w-4 text-red-500 mr-1"/>{currentPlayer.respostasErradas}</span>
                             <span className="flex items-center" title="Respostas Seguidas"><Zap className="h-4 w-4 text-orange-500 mr-1"/>{currentPlayer.respostasSeguidas}</span>
                         </div>
                     </div>
                 </CardFooter>
            </Card>
             ...
             <div className="mt-6 w-full max-w-lg">
                ...
                <div className={cn('grid gap-2', players.length > 4 ? (players.length > 6 ? "grid-cols-4" : "grid-cols-3") : `grid-cols-${Math.max(players.length, 1)}`)}>
        ...
        switch (cartaAtual.tipo) {
            case "Pergunta": case "ContraTempo": case "Vantagem": case "Desvantagem": case "Outras":
                ...
                return cartaAtual.opcoes.map((op: Opcao) => {
                    const isCorrect = Array.isArray(cartaAtual.respostaCorreta) ? cartaAtual.respostaCorreta.includes(op.id) : cartaAtual.respostaCorreta === op.id; const isSelected = selecionado === op.id; const isEliminated = opcoesEliminadas.includes(op.id); const isWrongSelection = respondido && isSelected && !isCorrect; let btnClass = "border-gray-300 text-gray-900 hover:bg-gray-100";
                    if (respondido) { if (isCorrect) btnClass = "bg-green-100 border-green-400 hover:bg-green-200 text-green-900"; else if (isSelected) btnClass = "bg-red-100 border-red-400 hover:bg-red-200 text-red-900"; else btnClass = "border-gray-300 text-gray-500"; } else if (isSelected) { btnClass = "bg-blue-100 border-blue-400 text-blue-900"; }
