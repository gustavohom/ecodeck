...
// Estado do Jogo (GameState) - Inclui mostrarSomentePerguntas
interface GameState {
    ...
    activeSourceIds: string[]; activeInternalBaralhosState: Record<string, string[]>;
    mostrarSomentePerguntas: boolean; // <<--- REINTEGRADO
}
// Estado da UI da Tela Inicial (Salvo separadamente)
interface TelaInicialUISettings {
    categoriasSelecionadas: string[]; ocultarCarta: boolean; probabilityIndex: number;
}
...
const LOCALSTORAGE_KEYS = {
    CUSTOM_SOURCES: "customSourceInfos", BUILTIN_STATUS: "builtInSourceStatus", ACTIVE_INTERNAL_BARALHOS: "activeInternalBaralhos",
    UI_SETTINGS: "ecoChallengeUISettings", GAME_STATE: "estadoEcoChallenge"
};
...
interface TelaInicialProps {
    ...
    initialPlayers: Player[]; initialOcultarCarta: boolean; initialProbabilityIndex: number; hasSavedGame: boolean;
}

const TelaInicial: React.FC<TelaInicialProps> = ({
    onStartGame, initialPlayers, initialOcultarCarta: initialOcultarCartaFromSave,
    ...
    const [playerInputs, setPlayerInputs] = useState<PlayerInput[]>(() => initialPlayers.length > 0 ? initialPlayers.map((p, index) => ({ id: p.id ?? index, name: p.name, color: p.color, showColorPicker: false })) : [{ id: Date.now(), name: "", color: predefinedColors[0], showColorPicker: false }] );
    ...
    const [categoriasAtivasComContagem, setCategoriasAtivasComContagem] = useState<Record<string, number>>({});
    ...
    useEffect(() => {
        setIsClient(true);
        const savedCustomRaw = localStorage.getItem(LOCALSTORAGE_KEYS.CUSTOM_SOURCES); let loadedCustomSources: SourceInfo[] = [];
        ...
        const savedBuiltInStatusRaw = localStorage.getItem(LOCALSTORAGE_KEYS.BUILTIN_STATUS); let loadedBuiltInStatus: Record<string, boolean> | null = null; try { loadedBuiltInStatus = savedBuiltInStatusRaw ? JSON.parse(savedBuiltInStatusRaw) : null; } catch (e) { console.error("Erro carregar builtInSourceStatus:", e); }
        const combinedSources = [ ...initialBuiltInSources.map(bs => ({...bs, active: loadedBuiltInStatus ? (loadedBuiltInStatus[bs.id] ?? true) : true})), ...processedCustomSources ]; setAllSources(combinedSources);
        const savedActiveInternalRaw = localStorage.getItem(LOCALSTORAGE_KEYS.ACTIVE_INTERNAL_BARALHOS); let initialActiveBaralhosState: Record<string, Set<string>> = {};
        ...
        combinedSources.forEach(source => { if (!initialActiveBaralhosState[source.id]) { initialActiveBaralhosState[source.id] = new Set(Object.keys(source.internalBaralhos)); } else { const currentInternalBaralhoNames = Object.keys(source.internalBaralhos); const savedBaralhoSet = initialActiveBaralhosState[source.id]; savedBaralhoSet.forEach(savedBName => { if (!source.internalBaralhos[savedBName]) { savedBaralhoSet.delete(savedBName); }}); if (currentInternalBaralhoNames.length === 0) { savedBaralhoSet.clear(); } } }); setActiveInternalBaralhos(initialActiveBaralhosState);
        const savedUISettingsRaw = localStorage.getItem(LOCALSTORAGE_KEYS.UI_SETTINGS); let loadedUISettings: Partial<TelaInicialUISettings> = {}; try { loadedUISettings = savedUISettingsRaw ? JSON.parse(savedUISettingsRaw) : {}; } catch (e) { console.error("Erro carregar UI Settings:", e); }
        setCategoriasSelecionadas(loadedUISettings.categoriasSelecionadas ?? []);
        setOcultarCarta(loadedUISettings.ocultarCarta ?? initialOcultarCartaFromSave ?? true);
        setProbabilityIndex(loadedUISettings.probabilityIndex ?? initialProbabilityIndexFromSave ?? 0);
    }, []);
    useEffect(() => {
        if (isClient) {
            const { categorias, contagens } = recalcularCategoriasAtivas(allSources, activeInternalBaralhos);
            setTodasCategorias(categorias); setCategoriasAtivasComContagem(contagens);
            setCategoriasSelecionadas((prev) => prev.filter(cat => categorias.includes(cat)));
        }
    }, [allSources, activeInternalBaralhos, isClient]);
    useEffect(() => {
        if (isClient) {
            const customSourcesToSave = allSources.filter(s => s.type === 'custom'); localStorage.setItem(LOCALSTORAGE_KEYS.CUSTOM_SOURCES, JSON.stringify(customSourcesToSave));
            const builtInStatusToSave: Record<string, boolean> = {}; allSources.filter(s => s.type === 'builtin').forEach(s => { builtInStatusToSave[s.id] = s.active; }); localStorage.setItem(LOCALSTORAGE_KEYS.BUILTIN_STATUS, JSON.stringify(builtInStatusToSave));
            const serializableActiveInternal = Object.entries(activeInternalBaralhos).reduce((acc, [key, valueSet]) => { acc[key] = Array.from(valueSet); return acc; }, {} as Record<string, string[]>); localStorage.setItem(LOCALSTORAGE_KEYS.ACTIVE_INTERNAL_BARALHOS, JSON.stringify(serializableActiveInternal));
            const uiSettings: TelaInicialUISettings = { categoriasSelecionadas: categoriasSelecionadas, ocultarCarta: ocultarCarta, probabilityIndex: probabilityIndex, }; localStorage.setItem(LOCALSTORAGE_KEYS.UI_SETTINGS, JSON.stringify(uiSettings));
        }
    }, [allSources, activeInternalBaralhos, categoriasSelecionadas, ocultarCarta, probabilityIndex, isClient]);
    ...
    const handleStartGame = (continueGame = false) => {
        ...
        if (continueGame && hasSavedGame && typeof window !== "undefined") {
            ...
            try { const savedState = savedStateRaw ? JSON.parse(savedStateRaw) as GameState : null; if (savedState && savedState.jogoIniciado) { gameStateToPass = { ...savedState, categoriasSelecionadas: finalCategoriasSelecionadas, ocultarCarta: ocultarCarta, probabilityIndex: probabilityIndex, activeSourceIds: sourcesForGame.map(s => s.id), activeInternalBaralhosState: activeInternalBaralhosStateForSave, jogoIniciado: true, mostrarSomentePerguntas: savedState.mostrarSomentePerguntas ?? false }; console.log("Continuando jogo:", gameStateToPass); } else { console.warn("Jogo salvo inválido. Iniciando novo."); return handleStartGame(false); } } catch (e) { console.error("Erro carregar jogo:", e); return handleStartGame(false); }
        } else {
            ...
            <CardContent className="space-y-6">
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <ScrollArea className="h-40 border rounded-md p-2 bg-gray-100 space-y-2">
                           {/* ... (renderização fontes built-in mantida) ... */}
                    ...
                    <div className="mt-4">
                        ...
                        {customSourcesUI.length > 0 ? (
                            <ScrollArea className="h-40 border rounded-md p-2 bg-gray-100 space-y-2">
                               {/* ... (renderização fontes custom mantida) ... */}
                ...
                <div className="space-y-2">
                    ...
                    <ScrollArea className="h-40 border rounded-md p-3 bg-gray-50">
                        {/* ... (renderização categorias mantida) ... */}
...
const EcoChallenge: React.FC = () => {
    ...
    // REINTEGRADO: Estado para o filtro de perguntas
    ...
    const handleGameStart = useCallback((initialGameState?: Partial<GameState>, sourcesForGame?: SourceInfo[]) => {
        if (initialGameState && initialGameState.jogoIniciado && sourcesForGame && sourcesForGame.length > 0) {
            setGameState(initialGameState as GameState); setCurrentGameSources(sourcesForGame);
            // REINTEGRADO: Define o estado inicial do filtro
            setMostrarSomentePerguntas(initialGameState.mostrarSomentePerguntas ?? false);
            // Reseta o resto
            setCartaAtual(null); setNoCardsAvailable(false); setRespondido(false); setMensagem(""); setMostrarDica(false); setDicaUsada(false); setMostrarFontes(false); setOpcoesEliminadas([]); setSelecionado(null); setSelecoesMultiplas([]); setOrdemSelecoes([]); setTempoRestante(null); setSelecaoColunaA(null); setParesFormados([]); setCoordenadasClique(null); setFragmentosSelecionados([]); setRolledNumber(null); setIsDieModalOpen(false); setCartaRevelada(!(initialGameState.ocultarCarta ?? true));
            console.log("Jogo iniciado/continuado. Jogadores:", initialGameState.players, "Filtro Perguntas:", initialGameState.mostrarSomentePerguntas);
        } else if (initialGameState?.jogoIniciado && initialGameState.activeSourceIds) {
    ...
    const updateGameState = useCallback((newState: Partial<GameState>) => {
        setGameState(prev => {
            if (!prev) return null;
            // Garante que mostrarSomentePerguntas seja incluído na atualização se não estiver explicitamente em newState
            const updatedState = {
                ...prev,
                ...newState,
                mostrarSomentePerguntas: newState.mostrarSomentePerguntas ?? prev.mostrarSomentePerguntas
            };
            if (typeof window !== "undefined") {
                try {
                    // Salva o estado COMPLETO, incluindo mostrarSomentePerguntas
                    localStorage.setItem(LOCALSTORAGE_KEYS.GAME_STATE, JSON.stringify(updatedState));
                } catch (e) { console.error("Erro ao salvar estado:", e); }
            }
            return updatedState;
        });
    }, []); // Removido mostrarSomentePerguntas daqui, pois é gerenciado internamente na função

    const updateCurrentPlayer = useCallback((partialPlayerData: Partial<Player>) => {
        if (!gameState || gameState.currentPlayerId === null) return;
        const playerExists = gameState.players.some(p => p.id === gameState.currentPlayerId); if (!playerExists) { console.error(`Jogador ${gameState.currentPlayerId} não encontrado.`); return; }
        const updatedPlayers = gameState.players.map(p => p.id === gameState.currentPlayerId ? { ...p, ...partialPlayerData } : p );
        // Chama updateGameState que agora salva o estado completo
        updateGameState({ players: updatedPlayers });
    }, [gameState, updateGameState]); // Mantém dependência de updateGameState
    ...
    useEffect(() => {
        if (gameState?.jogoIniciado && gameState.players.length > 0 && currentGameSources.length > 0 && !cartaAtual && !noCardsAvailable) { selecionarCartaAleatoria(); }
    }, [gameState?.jogoIniciado, gameState?.players, currentGameSources, cartaAtual, noCardsAvailable, selecionarCartaAleatoria]);

    // Efeito Timer ContraTempo
    useEffect(() => { /* ... (lógica mantida) ... */
        if (cartaAtual?.tipo === "ContraTempo" && tempoRestante !== null && tempoRestante > 0 && !respondido && gameState?.jogoIniciado && cartaRevelada) {
            timerIntervalRef.current = setInterval(() => {
                setTempoRestante((prev) => {
                    if (prev === null || prev <= 1) { clearInterval(timerIntervalRef.current!); timerIntervalRef.current = null; setRespondido(true); const penalidade = cartaAtual.dificuldade === 'dificil' ? 15 : 10; setMensagem(`Tempo esgotado! ${cartaAtual.desvantagem || 'Tente novamente.'}`); setGameState(currentGameState => { if (!currentGameState || currentGameState.currentPlayerId === null) return currentGameState; const updatedPlayers = currentGameState.players.map(p => p.id === currentGameState.currentPlayerId ? { ...p, respostasErradas: p.respostasErradas + 1, respostasSeguidas: 0, progresso: Math.max(0, p.progresso - penalidade) } : p ); const finalState = { ...currentGameState, players: updatedPlayers }; if (typeof window !== "undefined") { localStorage.setItem(LOCALSTORAGE_KEYS.GAME_STATE, JSON.stringify(finalState)); } return finalState; }); return 0; }
                    return prev - 1;
                });
            }, 1000);
        } else if (timerIntervalRef.current && (respondido || tempoRestante === 0 || !cartaRevelada || !gameState?.jogoIniciado)) { clearInterval(timerIntervalRef.current); timerIntervalRef.current = null; }
    ...
    const verificarResposta = () => { /* ... (lógica mantida) ... */
        ...
        if (cartaAtual.tipo === "ContraTempo" && timerIntervalRef.current) { clearInterval(timerIntervalRef.current); timerIntervalRef.current = null; }
         switch (cartaAtual.tipo) {
            ...
            case "Ordem": cor = Array.isArray(cartaAtual.respostaCorreta) && ordemSelecoes.length === cartaAtual.respostaCorreta.length && ordemSelecoes.toString() === cartaAtual.respostaCorreta.toString(); if (cor) pontosGanhos = 30; aplicarEfeitoPadrao = true; break;
            case "RelacionarColunas": if (!Array.isArray(cartaAtual.respostaCorreta) || !Array.isArray(cartaAtual.colunaA) || !Array.isArray(cartaAtual.colunaB)) { cor = false; break; } const paresFormadosStr = paresFormados.map(p => `${p.aId}-${p.bId}`).sort().join(','); const paresCorretosStr = cartaAtual.respostaCorreta.map(p => `${p.aId}-${p.bId}`).sort().join(','); cor = paresFormados.length === cartaAtual.respostaCorreta.length && paresFormadosStr === paresCorretosStr; if (cor) pontosGanhos = 30; aplicarEfeitoPadrao = true; break;
            case "PontoCerto": if (!coordenadasClique || !Array.isArray(cartaAtual.zonasClicaveis)) { cor = false; break; } const zonaCorreta = cartaAtual.zonasClicaveis.find(z => z.id === cartaAtual.respostaCorreta); cor = zonaCorreta ? isClickInZone(coordenadasClique, zonaCorreta) : false; if (cor) pontosGanhos = 25; aplicarEfeitoPadrao = true; break;
            case "CompletarFrase": if (!Array.isArray(cartaAtual.respostaCorreta) || !Array.isArray(cartaAtual.fragmentos)) { cor = false; break; } cor = fragmentosSelecionados.length === cartaAtual.respostaCorreta.length && fragmentosSelecionados.toString() === cartaAtual.respostaCorreta.toString(); if (cor) pontosGanhos = 25; aplicarEfeitoPadrao = true; break;
            case "Vantagem": cor = selecionado !== null && Array.isArray(cartaAtual.respostaCorreta) && cartaAtual.respostaCorreta.includes(selecionado); if (selecionado === null) { mensagemFinal = "Nenhuma opção selecionada."; } else { mensagemFinal = cor ? (cartaAtual.vantagem || "Vantagem aplicada!") : "Opção inválida."; } aplicarEfeitoPadrao = false; break;
            case "Desvantagem": cor = false; mensagemFinal = cartaAtual.desvantagem || "Desvantagem aplicada."; aplicarEfeitoPadrao = false; break;
            case "Outras": cor = selecionado !== null && Array.isArray(cartaAtual.respostaCorreta) && cartaAtual.respostaCorreta.includes(selecionado); if (selecionado === null) { mensagemFinal = "Nenhuma opção selecionada."; } else { mensagemFinal = cor ? (cartaAtual.vantagem || 'Ok!') : (cartaAtual.desvantagem || 'Hmm...'); } aplicarEfeitoPadrao = false; break;
        ...
        if (aplicarEfeitoPadrao) {
            ...
            if (cor) { const novoProgresso = Math.min(100, currentPlayer.progresso + pontosGanhos); const completouBarra = novoProgresso >= 100; const pulosGanhosBase = completouBarra ? 1 : 0; const pulosGanhosExtra = cartaAtual.dificuldade === 'dificil' ? 1 : 0; const estrelasFixasGanhsa = completouBarra ? 1 : 0; updateCurrentPlayer({ respostasCertas: currentPlayer.respostasCertas + 1, respostasSeguidas: currentPlayer.respostasSeguidas + 1, progresso: completouBarra ? 0 : novoProgresso, pulosDisponiveis: Math.min(currentPlayer.pulosDisponiveis + pulosGanhosBase + pulosGanhosExtra, 2), fixedStars: currentPlayer.fixedStars + estrelasFixasGanhsa, }); mensagemFinal = `Correto! ${cartaAtual.vantagem || ''}${completouBarra ? ' Barra completa! (+1 Progresso, +1 Pulo)' : ''}${!completouBarra && pulosGanhosExtra > 0 ? ' (+1 Pulo)' : ''}`; }
            else { updateCurrentPlayer({ respostasErradas: currentPlayer.respostasErradas + 1, respostasSeguidas: 0, progresso: Math.max(0, currentPlayer.progresso - pontosPerdidos), }); let detalheErro = ""; if (cartaAtual.tipo === 'Pergunta' && 'respostaCorreta' in cartaAtual && 'opcoes' in cartaAtual) { const opCorreta = cartaAtual.opcoes.find(o => o.id === cartaAtual.respostaCorreta); if (opCorreta) detalheErro = ` Correto: ${opCorreta.texto}.`; } else if (cartaAtual.tipo === 'MultiplaEscolha' && Array.isArray(cartaAtual.respostaCorreta) && 'opcoes' in cartaAtual) { const opsCorretas = cartaAtual.opcoes.filter(o => cartaAtual.respostaCorreta.includes(o.id)).map(o => o.texto).join(', '); if (opsCorretas) detalheErro = ` Correto: ${opsCorretas}.`; } mensagemFinal = mensagemFinal || `Incorreto. ${cartaAtual.desvantagem || ''}${detalheErro}`; }
        }
    ...
    const eliminarRespostaErrada = () => { const cp = gameState?.players.find(p => p.id === gameState?.currentPlayerId); if (!cp || !cartaAtual || respondido || (gameState?.ocultarCarta && !cartaRevelada)) return; const tiposEliminaveis: Carta['tipo'][] = ["Pergunta", "MultiplaEscolha", "ContraTempo", "Outras"]; if (!tiposEliminaveis.includes(cartaAtual.tipo) || !('opcoes' in cartaAtual) || !Array.isArray(cartaAtual.opcoes)) { setMensagem("Não pode eliminar opções."); return; } const opcoesAtivas = cartaAtual.opcoes.filter(op => !opcoesEliminadas.includes(op.id)); if (opcoesAtivas.length <= 2) { setMensagem("Não pode eliminar mais."); return; } if (cp.respostasSeguidas < 2) { setMensagem("Necessário 2 acertos seguidos."); return; } let respostaCorretaIds: number[] = []; if ('respostaCorreta' in cartaAtual) { if (Array.isArray(cartaAtual.respostaCorreta)) { respostaCorretaIds = cartaAtual.respostaCorreta.filter(id => typeof id === 'number'); } else if (typeof cartaAtual.respostaCorreta === 'number') { respostaCorretaIds = [cartaAtual.respostaCorreta]; } } const opcoesErradasDisp = cartaAtual.opcoes.filter(op => !respostaCorretaIds.includes(op.id) && !opcoesEliminadas.includes(op.id)); if (opcoesErradasDisp.length > 0) { const idxAleat = Math.floor(Math.random() * opcoesErradasDisp.length); const opcaoEliminadaId = opcoesErradasDisp[idxAleat].id; setOpcoesEliminadas((prev) => [...prev, opcaoEliminadaId]); updateCurrentPlayer({ respostasSeguidas: cp.respostasSeguidas - 2 }); setMensagem("Opção incorreta eliminada! (-2 Seguidos)"); } else { setMensagem("Não há mais incorretas para eliminar."); } };
    ...
    const handleLongPressStart = (action: () => void) => { longPressTimeout.current = setTimeout(() => { rolarDado(); }, 700); };
    ...
    const { players, currentPlayerId, ocultarCarta: isCartaOculta, mostrarSomentePerguntas: filtroPerguntasAtivo } = gameState; // Pega filtro do estado
    const currentPlayer = players.find(p => p.id === currentPlayerId);
    if (noCardsAvailable) { /* ... renderização sem cartas ... */
        return ( <div className="flex flex-col items-center justify-center min-h-screen p-4 text-center"><Card className="p-6 shadow-lg max-w-sm"><CardHeader><CardTitle className="text-xl text-red-600">Sem Cartas!</CardTitle></CardHeader><CardContent><p className="mb-4">Nenhuma carta encontrada.</p><p className="text-sm text-gray-600 mb-4">Ajuste filtros/baralhos na tela inicial.</p></CardContent><CardFooter><Button onClick={voltarTelaInicial} className="w-full">Voltar</Button></CardFooter></Card></div> );
    }
    if (!cartaAtual || !currentPlayer) { /* ... renderização carregando ... */
         return ( <div className="flex flex-col items-center justify-center min-h-screen"><p className="mb-4">Carregando...</p><Button onClick={voltarTelaInicial} variant="outline">Voltar (Forçar)</Button></div> );
    }
            ...
            >
                ...
                <CardHeader className="pb-3">
                    <div className="flex justify-between items-start mb-2 gap-2">
                        {/* REINTEGRADO: Botão de Filtro + Título/Categoria */}
                        <div className="flex items-start space-x-2 flex-1 min-w-0">
                            ...
                            <Button
                                ...
                                className={cn(
                                    "flex-shrink-0 h-9 w-9 p-0 mt-0.5", // Alinhamento sutil com título
                                    filtroPerguntasAtivo && "ring-2 ring-offset-1 ring-blue-500 bg-blue-100 border-blue-300"
                                )}
                            >
                                <Filter className="h-4 w-4" />
                            </Button>
                             {/* Título e Categoria/Baralho */}
                            <div className="flex-1 min-w-0">
                                ...
                                {(!isCartaOculta || cartaRevelada) && (
                                    <p
                                        ...
                                        title={(cartaAtual.categorias ? cartaAtual.categorias.join(", ") : 'Sem Categoria') + (cartaAtual.baralho && cartaAtual.baralho !== DEFAULT_BARALHO_NAME ? ` (Baralho: ${cartaAtual.baralho})` : '')}
                                    >
                        ...
                        {(!isCartaOculta || cartaRevelada) && (
                            <Badge
                                ...
                                className="capitalize flex-shrink-0 h-6 px-2.5 text-xs ml-2" // Adicionado ml-2 para espaçamento
                            >
    ...
    function renderizarConteudoResposta() {
        ...
        switch (cartaAtual.tipo) { // ... (Cases mantidos) ...
