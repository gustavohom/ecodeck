...
const EcoChallenge: React.FC = () => {
    ...
    const verificarResposta = () => { /* ... (lógica mantida) ... */
        ...
        if (aplicarEfeitoPadrao) {
            ...
            if (cor) { const novoProgresso = Math.min(100, currentPlayer.progresso + pontosGanhos); const completouBarra = novoProgresso >= 100; const pulosGanhosBase = completouBarra ? 1 : 0; const pulosGanhosExtra = darPuloDificil ? 1 : 0; const estrelasFixasGanhsa = completouBarra ? 1 : 0; updateCurrentPlayer({ respostasCertas: currentPlayer.respostasCertas + 1, respostasSeguidas: currentPlayer.respostasSeguidas + 1, progresso: completouBarra ? 0 : novoProgresso, pulosDisponiveis: Math.min(currentPlayer.pulosDisponiveis + pulosGanhosBase + pulosGanhosExtra, 2), fixedStars: currentPlayer.fixedStars + estrelasFixasGanhsa, }); mensagemFinal = `Correto! ${cartaAtual.vantagem || ''}${completouBarra ? ' Barra completa! (+1 Estrela Fixa, +1 Pulo)' : ''}${!completouBarra && pulosGanhosExtra > 0 ? ' (+1 Pulo)' : ''}`; }
