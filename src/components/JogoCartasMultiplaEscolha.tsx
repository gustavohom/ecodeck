...
interface TelaInicialProps {
    onStartGame: (initialState?: Partial<GameState>) => void; categoriasDisponiveis: string[];
    initialCategoriasSelecionadas: string[]; initialPlayers: Player[]; initialOcultarCarta: boolean;
    initialProbabilityIndex: number; hasSavedGame: boolean;
}
interface GameState {
    players: Player[]; currentPlayerId: number | null; categoriasSelecionadas: string[]; ocultarCarta: boolean;
    probabilityIndex: number; jogoIniciado: boolean; customDecks: CustomDeck[]; usedDeckIds: number[];
}
...
const TelaInicial: React.FC<TelaInicialProps> = ({
    ...
    useEffect(() => {
        const allCats = recalcularCategorias(cartasOriginais, customDecks); setTodasCategorias(allCats);
        setCategoriasSelecionadas((prevCats) => prevCats.filter((c) => allCats.includes(c)));
    }, [customDecks]);
    useEffect(() => { if (typeof window !== "undefined") { localStorage.setItem("customDecks", JSON.stringify(customDecks)); } }, [customDecks]);
        ...
        setGameState(prev => {
            ...
            if (typeof window !== "undefined") {
                try {
                    ...
                    localStorage.setItem("estadoEcoChallenge", JSON.stringify({...stateToSave, mostrarSomentePerguntas}));
                            ...
                            if (localCurrentPlayer) {
                                const updatedPlayers = currentGameState.players.map(p =>
                                    p.id === currentGameState.currentPlayerId
                                        ? { ...p, respostasErradas: p.respostasErradas + 1, respostasSeguidas: 0, progresso: Math.max(p.progresso - 10, 0) } : p );
                                return { ...currentGameState, players: updatedPlayers };
            ...
            >
                ...
                <CardFooter className="flex flex-col items-center pt-4 border-t bg-gray-50/50 rounded-b-lg">
                    ...
                    <div className="flex flex-wrap justify-center gap-1.5 w-full mb-3">
                        ...
                        <Button onClick={eliminarRespostaErrada} size="sm" variant={currentPlayer.respostasSeguidas >= 2 ? "secondary" : "outline"} disabled={currentPlayer.respostasSeguidas < 2 || !["Pergunta", "MultiplaEscolha", "ContraTempo", "Outras"].includes(cartaAtual.tipo) || respondido || (ocultarCarta && !cartaRevelada)} className="h-8 px-2"> <MinusCircle className="h-4 w-4" /> </Button>
                    ...
                    {/* Botões de Ajuste Manual - Tamanho Aumentado */}
                    <div className="flex flex-wrap justify-center gap-1.5 w-full mb-3">
                    ...
                    <div className="w-full">
                        ...
                        <div className="flex justify-between items-center w-full mt-2 text-sm text-gray-700 flex-wrap gap-x-4 gap-y-1 font-medium"> {/* Aumentado tamanho fonte/gap */}
                            ...
                            <span className="flex items-center" title="Respostas Seguidas"><Zap className="h-4 w-4 text-orange-500 mr-0.5"/>{currentPlayer.respostasSeguidas}</span>
                        </div>
                    </div>
                </CardFooter>
            </Card>
