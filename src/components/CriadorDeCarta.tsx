...
import { Trash, Youtube, Image as ImageIcon, Video, ArrowUpCircle } from "lucide-react"; // Importar Ícones + ArrowUpCircle
import { cn } from "@/lib/utils";

// --- Tipos de Dados (com baralho: string) ---
...
interface CartaBase {
    ...
    opcoes?: Opcao[];
    baralho?: string; // <<--- REVERTIDO para string opcional
}
// ... (Restante das interfaces de carta mantidas) ...
...
const DEFAULT_BARALHO_NAME = "Padrão"; // Constante já existente
...
// --- Componente de Preview Estático (Atualizado para incluir Baralho) ---
const CardStaticView: React.FC<{ card: Partial<Carta> }> = ({ card }) => {
    ...
    // ... (lógica switch case para tipos específicos mantida) ...
    switch (tipo) {
        case "ContraTempo":
     ...
     if (["Pergunta", "MultiplaEscolha", "Ordem", "Vantagem", "Desvantagem", "Outras", "ContraTempo"].includes(tipo)) {
        // ... (lógica de opções mantida) ...
        const correctSet = new Set<number>();
        const currentOptions = opcoes || [];
        if (tipo === "Vantagem") { currentOptions.forEach(o => correctSet.add(o.id)); }
        else if (tipo !== "Desvantagem") { if (Array.isArray(respostaCorreta)) { (respostaCorreta as number[]).forEach(id => typeof id === 'number' && correctSet.add(id)); } else if (typeof respostaCorreta === 'number') { correctSet.add(respostaCorreta); } }
        renderedOptions = ( /* ... mantido ... */
        ...
        <Card className="max-w-md mx-auto my-4 shadow-md border">
            <CardHeader className="pb-2 bg-slate-50 rounded-t-lg">
                ...
                <div className="flex justify-between items-center text-xs text-gray-500 pt-1">
                    ...
                    {/* Exibe o baralho se não for o padrão */}
...
const CriadorDeCarta: React.FC = () => {
    ...
    const [baralhoCartaAtual, setBaralhoCartaAtual] = useState(DEFAULT_BARALHO_NAME); // <<--- NOVO ESTADO
    ...
    // Preview Ponto Certo
    useEffect(() => { /* ... (mantido) ... */
        if (tipo === 'PontoCerto' && previewImageRefPontoCerto.current) { const updateSize = () => { if (previewImageRefPontoCerto.current) { setPreviewImageSize({ width: previewImageRefPontoCerto.current.offsetWidth, height: previewImageRefPontoCerto.current.offsetHeight }); } }; const img = previewImageRefPontoCerto.current; img.onload = updateSize; if (img.complete) updateSize(); const observer = new ResizeObserver(updateSize); observer.observe(img); window.addEventListener('resize', updateSize); return () => { window.removeEventListener('resize', updateSize); observer.disconnect(); img.onload = null;}; }
    }, [tipo, imagemURLPontoCerto]);

    // Mostrar/Ocultar botão "Voltar ao Topo"
     useEffect(() => {
        const handleScroll = () => {
            setShowScrollTop(window.scrollY > 200); // Mostra se scrollar mais de 200px
        };
    ...
    // --- Funções Auxiliares ---
    const parseJSDeckFileLocal = (content: string): Carta[] => { /* ... (mantida) ... */
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => { /* ... (mantida) ... */
    ...
    const adicionarBaralho = (baralhoId: number) => { /* ... (mantida) ... */
    ...
    const removerBaralho = (baralhoId: number) => { /* ... (mantida) ... */
         setBaralhosCarregados((prev) => prev.map((b) => {
            if (b.id === baralhoId && b.adicionado) {
    ...
    // Atualizado para incluir baralhoCartaAtual
    const resetCarta = () => {
        ...
        setBaralhoCartaAtual(DEFAULT_BARALHO_NAME); // <<--- RESET ADICIONADO
    ...
    const handleAddOrUpdateCard = () => {
        ...
        const nomeBaralhoFinal = baralhoCartaAtual.trim() || DEFAULT_BARALHO_NAME; // <<--- Pega e trata o nome do baralho

        try { // ... (lógica switch case mantida) ...
            switch (tipo) {
                case "Pergunta": case "ContraTempo": if (respostaCorreta.length !== 1) throw new Error(`Tipo ${tipo} exige uma resposta correta.`); finalRespostaCorreta = respostaCorreta[0]; cartaEspecificaProps.opcoes = opcoes; if(tipo === 'ContraTempo') cartaEspecificaProps.tempoLimite = tempoLimite; break;
                case "MultiplaEscolha": case "Outras": case "Vantagem": case "Desvantagem": finalRespostaCorreta = (tipo === "Vantagem") ? opcoes.map(o => o.id) : (tipo === "Desvantagem" ? [] : respostaCorreta); cartaEspecificaProps.opcoes = opcoes; break;
                case "Ordem": const posicoes = new Map<number, number>(); let maxPos = 0; const ordemIds: number[] = []; let ordemValida = true; opcoes.forEach(op => { const pos = op.ordemTemp?.trim() ? parseInt(op.ordemTemp, 10) : NaN; if (isNaN(pos) || pos <= 0 || posicoes.has(pos)) { ordemValida = false; } else { posicoes.set(pos, op.id); maxPos = Math.max(maxPos, pos); } }); if (!ordemValida || posicoes.size !== opcoes.length || maxPos !== opcoes.length) { throw new Error("Erro na ordem: posições devem ser únicas de 1 a N."); } for (let i = 1; i <= maxPos; i++) { ordemIds.push(posicoes.get(i)!); } finalRespostaCorreta = ordemIds; cartaEspecificaProps.opcoes = opcoes.map(({ordemTemp, ...rest}) => rest); break;
                case "RelacionarColunas": try { finalRespostaCorreta = parseParesRelacionar(paresCorretosInput); if (colunaAItems.length === 0 || colunaBItems.length === 0) { throw new Error("Adicione itens às Colunas A e B."); } if (finalRespostaCorreta.length === 0 && paresCorretosInput.trim() !== '') { throw new Error("Formato inválido para Pares Corretos. Use IDa-IDb, IDa-IDb."); } } catch (error: any) { throw new Error(`Erro ao processar Pares Corretos: ${error.message}`); } cartaEspecificaProps.colunaA = colunaAItems; cartaEspecificaProps.colunaB = colunaBItems; break;
                case "PontoCerto": if (!imagemURLPontoCerto) throw new Error("URL da Imagem é obrigatória."); if (zonasClicaveis.length === 0) throw new Error("Adicione pelo menos uma Zona Clicável."); if (respostaCorretaPontoCerto === null || !zonasClicaveis.some(z => z.id === respostaCorretaPontoCerto)) throw new Error("Selecione uma Zona Correta válida."); finalRespostaCorreta = respostaCorretaPontoCerto; cartaEspecificaProps.imagemURL = imagemURLPontoCerto; cartaEspecificaProps.zonasClicaveis = zonasClicaveis; break;
                case "CompletarFrase": if (!fraseIncompleta.trim()) throw new Error("Frase Incompleta é obrigatória."); if (fragmentos.length === 0) throw new Error("Adicione pelo menos um Fragmento."); const ordemIdsFrag = ordemFragmentos.split(',').map(s => parseInt(s.trim(), 10)).filter(n => !isNaN(n)); if (ordemIdsFrag.length === 0 || !ordemIdsFrag.every(id => fragmentos.some(f => f.id === id))) throw new Error("Ordem dos Fragmentos inválida ou IDs não encontrados."); finalRespostaCorreta = ordemIdsFrag; cartaEspecificaProps.fraseIncompleta = fraseIncompleta; cartaEspecificaProps.fragmentos = fragmentos; break;
        ...
        const cartaBaseProps = {
             ...
             vantagem, desvantagem, dica,
             baralho: nomeBaralhoFinal, // <<--- BARALHO ADICIONADO AQUI
    ...
    const loadCardForEdit = (index: number) => {
        ...
        setBaralhoCartaAtual(carta.baralho || DEFAULT_BARALHO_NAME); // <<--- CARREGAR BARALHO
        ...
        // ... (restante da lógica switch case mantida) ...
        switch (carta.tipo) {
            case "Pergunta": case "MultiplaEscolha": case "Vantagem": case "Desvantagem": case "Outras": case "ContraTempo": setOpcoes(carta.opcoes ? [...carta.opcoes] : []); const rcArray = Array.isArray(carta.respostaCorreta) ? carta.respostaCorreta : (typeof carta.respostaCorreta === 'number' ? [carta.respostaCorreta] : []); setRespostaCorreta(rcArray.filter(id => typeof id === 'number')); if(carta.tipo === 'ContraTempo') setTempoLimite((carta as CartaContraTempo).tempoLimite || 30); break;
            case "Ordem": setOpcoes((carta as CartaOrdem).opcoes?.map(op => ({...op})) || []); if (Array.isArray(carta.respostaCorreta) && carta.opcoes) { const ordemCorreta = carta.respostaCorreta as number[]; setOpcoes(currentOpts => currentOpts.map(op => ({...op, ordemTemp: ordemCorreta.indexOf(op.id) >= 0 ? String(ordemCorreta.indexOf(op.id) + 1) : "" }))); } setRespostaCorreta([]); break;
            case "RelacionarColunas": const cRel = carta as CartaRelacionarColunas; setColunaAItems(cRel.colunaA ? [...cRel.colunaA] : []); setColunaBItems(cRel.colunaB ? [...cRel.colunaB] : []); const paresStringFormat = Array.isArray(cRel.respostaCorreta) ? cRel.respostaCorreta.map(p => `${p.aId}-${p.bId}`).join(', ') : ""; setParesCorretosInput(paresStringFormat); break;
            case "PontoCerto": const cPonto = carta as CartaPontoCerto; setImagemURLPontoCerto(cPonto.imagemURL || ""); setZonasClicaveis(cPonto.zonasClicaveis ? [...cPonto.zonasClicaveis] : []); setRespostaCorretaPontoCerto(typeof cPonto.respostaCorreta === 'number' ? cPonto.respostaCorreta : null); break;
            case "CompletarFrase": const cFrase = carta as CartaCompletarFrase; setFraseIncompleta(cFrase.fraseIncompleta || ""); setFragmentos(cFrase.fragmentos ? [...cFrase.fragmentos] : []); setOrdemFragmentos(Array.isArray(cFrase.respostaCorreta) ? cFrase.respostaCorreta.join(', ') : ""); break;
        }
    };
    const deleteCard = (index: number) => { /* ... (mantida) ... */
         setCards((old) => old.filter((_, i) => i !== index)); if (editIndex === index) { resetCarta(); }
    };
    const handleRemoveAllCards = () => { /* ... (mantida) ... */
        if (window.confirm(`Remover TODAS as ${cards.length} cartas?`)) { setCards([]); setEditIndex(null); resetCarta(); }
    };
    ...
    const prepareForDownload = (): Partial<Carta>[] => {
        return cards.map(({ origBaralhoId, edited, ...rest }: CartaInterna) => {
            ...
            // Remove campos específicos de outros tipos
            ...
            // Remove 'opcoes' se não for um tipo que as usa
            if (!["Pergunta", "MultiplaEscolha", "Ordem", "Vantagem", "Desvantagem", "Outras", "ContraTempo"].includes(rest.tipo) || cardData.opcoes === undefined ) { delete cardData.opcoes; }
            // Remove 'ordemTemp' das opções de Ordem
            else if (rest.tipo === "Ordem" && cardData.opcoes) { cardData.opcoes = cardData.opcoes.map(({ ordemTemp, ...o }) => o); }
            // Remove 'baralho' se for o padrão
            if (cardData.baralho === DEFAULT_BARALHO_NAME) { delete cardData.baralho; }
            return cardData;
        });
    };
    const generateCode = (format: 'js' | 'json') => { /* ... (mantida) ... */
         const deckFinal = prepareForDownload(); if (format === 'json') { return JSON.stringify(deckFinal, null, 2); } else { const deck = JSON.stringify(deckFinal, null, 2); return `const ${deckName || 'meu_baralho'} = ${deck};\n\nexport default ${deckName || 'meu_baralho'};`; }
    };
    const downloadCode = (format: 'js' | 'json') => { /* ... (mantida) ... */
         const element = document.createElement("a"); const fileContent = generateCode(format); const fileType = format === 'js' ? 'text/javascript' : 'application/json'; const fileName = `${deckName || 'meu_baralho'}.${format}`; const file = new Blob([fileContent], { type: fileType }); element.href = URL.createObjectURL(file); element.download = fileName; document.body.appendChild(element); element.click(); document.body.removeChild(element);
    };
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div>
                            ...
                            <Input type="text" value={deckName} onChange={(e) => setDeckName(e.target.value.replace(/[^a-zA-Z0-9_]/g, '_'))} placeholder={DEFAULT_BARALHO_NAME}/>
                        </div>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <Card className="lg:col-span-2 max-h-[90vh] overflow-y-auto self-start shadow-lg border">
                    ...
                    <CardContent className="space-y-5 p-4 md:p-6">
                         {/* Campos Comuns (Título, Tipo, Baralho, Dificuldade) */}
                        <Card className="border bg-slate-50 p-4 rounded-lg shadow-sm">
                             <CardContent className="p-0 space-y-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label htmlFor="card-baralho" className="block text-sm font-medium mb-1">Baralho (Opcional)</label>
                                        <Input id="card-baralho" type="text" value={baralhoCartaAtual} onChange={(e) => setBaralhoCartaAtual(e.target.value)} placeholder={DEFAULT_BARALHO_NAME} />
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        {/* --- Campos Condicionais --- */}
                         {["Pergunta", "MultiplaEscolha", "Ordem", "Vantagem", "Desvantagem", "Outras", "ContraTempo"].includes(tipo) && (
                            <Card className="bg-gray-50 border p-4 rounded-lg shadow-sm">
                        ...
                        {tipo === "ContraTempo" && ( /* ... (mantido) ... */
                            <Card className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg shadow-sm"> <CardContent className="p-0"> <label className="block text-sm font-medium mb-1">Tempo Limite (segundos)</label> <Input type="number" value={tempoLimite} onChange={(e) => setTempoLimite(Math.max(5, parseInt(e.target.value, 10) || 5))} min="5" className="w-24"/> </CardContent> </Card>
                        )}

                        {tipo === "RelacionarColunas" && ( /* ... (mantido, com melhorias de UI) ... */
                            <Card className="bg-blue-50 border border-blue-200 p-4 rounded-lg shadow-sm">
                        ...
                        {tipo === "PontoCerto" && ( /* ... (mantido, com melhorias de UI) ... */
                             <Card className="bg-indigo-50 border border-indigo-200 p-4 rounded-lg shadow-sm">
                        ...
                        {tipo === "CompletarFrase" && ( /* ... (mantido, com melhorias de UI) ... */
                            <Card className="bg-pink-50 border border-pink-200 p-4 rounded-lg shadow-sm">
                             ...
                             <CardContent className="p-0 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <div className="flex items-center justify-between mb-2">
                                        ...
                                        <div className="flex items-center space-x-1"> <Checkbox id="lockCat" checked={categoriasBloqueadas} onCheckedChange={checked => setCategoriasBloqueadas(Boolean(checked))} /> <label htmlFor="lockCat" className="text-xs">Travar</label> </div>
                                    </div>
                                 ...
                                 <div>
                                     <div className="flex items-center justify-between mb-2">
                                          ...
                                          <div className="flex items-center space-x-1"> <Checkbox id="lockFont" checked={fontesBloqueadas} onCheckedChange={checked => setFontesBloqueadas(Boolean(checked))} /> <label htmlFor="lockFont" className="text-xs">Travar</label> </div>
                                     </div>
                        ...
                        <Card className="border bg-slate-50 p-4 rounded-lg shadow-sm">
                             <CardContent className="p-0 space-y-3">
                                <div><label htmlFor="card-vantagem" className="block text-sm font-medium mb-1">Vantagem (Msg Acerto)</label><Input id="card-vantagem" type="text" value={vantagem} onChange={(e) => setVantagem(e.target.value)} /></div>
                                <div><label htmlFor="card-desvantagem" className="block text-sm font-medium mb-1">Desvantagem (Msg Erro)</label><Input id="card-desvantagem" type="text" value={desvantagem} onChange={(e) => setDesvantagem(e.target.value)} /></div>
                                <div><label htmlFor="card-dica" className="block text-sm font-medium mb-1">Dica</label><Input id="card-dica" type="text" value={dica} onChange={(e) => setDica(e.target.value)} /></div>
                            </CardContent>
                        </Card>
              ...
              {showScrollTop && (
                <Button
                    onClick={scrollToTop}
                    className="fixed bottom-5 right-5 z-50 h-12 w-12 rounded-full p-0 shadow-lg bg-gray-700 hover:bg-gray-900"
