...
const CriadorDeCarta: React.FC = () => {
    ...
    const [baralhoCartaAtual, setBaralhoCartaAtual] = useState("");
    const [baralhoBloqueado, setBaralhoBloqueado] = useState(false);
    ...
    // --- UseEffects (mantidos) ---
    useEffect(() => { /* Preview Ponto Certo */ }, [tipo, imagemURLPontoCerto]);
    ...
    const parseJSDeckFileLocal = (content: string): Carta[] => {
        try {
            ...
            // Cuidado: `new Function` pode ser um risco de segurança. Considere alternativas se a fonte não for confiável.
            ...
            console.error("Erro parse JS (local):", error);
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setIsLoading(true); setErrorMessage(null); let loadErrors: string[] = []; const newBaralhos: BaralhoCarregado[] = [];
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                ...
                if (file.name.endsWith(".js")) {
                    newCards = parseJSDeckFileLocal(content); // Chama a função local
                } else if (file.name.endsWith(".json")) {
                    newCards = JSON.parse(content) as Carta[];
                } else {
                    loadErrors.push(`Formato ${file.name} não suportado.`); continue;
                }
    ...
    // ... (Restante das funções como adicionarBaralho, removerBaralho, handlers, etc., permanecem iguais à resposta anterior)
    ...
    const adicionarBaralho = (baralhoId: number) => {
        ...
        if (!Array.isArray(baralhoParaAdicionar.cartas)) { console.error("Cartas inválidas:", baralhoParaAdicionar.nome); return; }

        try {
            const newCards: CartaInterna[] = baralhoParaAdicionar.cartas.map((c, index) => {
                const uniqueId = c.id || `${baralhoParaAdicionar.nome}_added_${index}_${Date.now()}_${Math.random().toString(16).slice(2)}`;
                const cartaInternaBase = {
                    ...c, id: uniqueId, tipo: c.tipo || "Pergunta", titulo: c.titulo || "", pergunta: c.pergunta || "",
                    dificuldade: c.dificuldade || "facil", categorias: Array.isArray(c.categorias) ? c.categorias : [],
                    fontes: Array.isArray(c.fontes) ? c.fontes : [], vantagem: c.vantagem || "", desvantagem: c.desvantagem || "",
                    dica: c.dica || "", baralho: c.baralho?.trim() || undefined,
        ...
        } catch (error) { console.error("Erro ao adicionar cartas:", error); setErrorMessage(`Erro ao processar cartas de "${baralhoParaAdicionar.nome}".`); }
    };
    ...
    const removerBaralho = (baralhoId: number) => {
        ...
        setCards((oldCards) => oldCards.filter((c) => c.origBaralhoId !== baralhoId || (c.edited && manterCartasEditadas) ));
        setBaralhosCarregados((prev) => prev.map((b) => b.id === baralhoId ? { ...b, adicionado: false } : b ));
        setErrorMessage(null);
    };

    // Handlers dos campos (mantidos)
    ...
    // Inserir Popup (mantido com indentação corrigida)
    const inserirTemplatePopup = (tipoPopup: 'imagem' | 'video') => {
        ...
        if (tipoPopup === 'imagem') { if (!popupImageUrl.trim()) { alert("Insira a URL da Imagem."); return; } urlPrincipal = popupImageUrl; urlThumb = popupImageUrl; desc = 'Descrição da Imagem'; thumbHtml = `<img src="${urlThumb}" alt="Clique para ampliar"/>`; templateElemento = `\n<!-- Link/Thumb Img -->\n<a href="#${idUnico}" class="thumb-link-${idUnico}">${thumbHtml}</a>\n<!-- Popup Img -->\n<div id="${idUnico}" class="popup-overlay-${idUnico}"><div class="popup-content-${idUnico}"><a href="#" class="popup-close-${idUnico}" title="Fechar">×</a><img src="${urlPrincipal}" alt="Imagem Ampliada"/><p style="text-align: center; font-size: 0.9em; color: #666;">${desc}</p></div></div>\n`; }
        else { if (!popupVideoUrl.trim()) { alert("Insira a URL do Vídeo (.mp4)."); return; } urlPrincipal = popupVideoUrl; desc = 'Descrição do Vídeo'; thumbHtml = `<span>🎬 Clique para ver o vídeo</span>`; templateElemento = `\n<!-- Link/Thumb Vídeo -->\n<a href="#${idUnico}" class="thumb-link-${idUnico}" style="border:none; background:none; padding:0;">${thumbHtml}</a>\n<!-- Popup Vídeo -->\n<div id="${idUnico}" class="popup-overlay-${idUnico}"><div class="popup-content-${idUnico}"><a href="#" id="close-btn-${idUnico}" class="popup-close-${idUnico}" title="Fechar">×</a><video controls width="100%" style="max-width: 700px; max-height: 70vh;" id="video-${idUnico}"><source src="${urlPrincipal}" type="video/mp4">Seu navegador não suporta vídeo.</video><p style="text-align: center; font-size: 0.9em; color: #666;">${desc}</p></div></div>\n`; scriptStop = `\n<script>(function() { var c=document.getElementById('close-btn-${idUnico}'),v=document.getElementById('video-${idUnico}');if(c&&v){c.addEventListener('mousedown',function(){if(!v.paused)v.pause();setTimeout(function(){window.location.hash='#_';},10);});}})();</script>\n`; }
    ...
    const handleAddOrUpdateCard = () => {
        ...
        const nomeBaralhoFinal = baralhoCartaAtual.trim() || undefined;

        try {
            switch (tipo) {
                case "Pergunta": case "ContraTempo":
                    if (respostaCorreta.length !== 1) throw new Error(`Tipo ${tipo} exige uma resposta correta.`);
                    finalRespostaCorreta = respostaCorreta[0]; cartaEspecificaProps.opcoes = opcoes;
                ...
                case "Ordem":
                    ...
                    finalRespostaCorreta = ordemIds; cartaEspecificaProps.opcoes = opcoes.map(({ordemTemp, ...rest}) => rest);
                    break;
        ...
        const cartaBaseProps = {
             ...
             baralho: nomeBaralhoFinal, respostaCorreta: finalRespostaCorreta, edited: true,
    ...
    const loadCardForEdit = (index: number) => {
        ...
        setBaralhoCartaAtual(carta.baralho || "");
    ...
    const prepareForDownload = (): Partial<Carta>[] => { return cards.map(({ origBaralhoId, edited, ...rest }: CartaInterna) => { const cardData: Partial<Carta> = { ...rest }; if (rest.tipo !== "ContraTempo") delete (cardData as Partial<CartaContraTempo>).tempoLimite; if (rest.tipo !== "RelacionarColunas") { delete (cardData as Partial<CartaRelacionarColunas>).colunaA; delete (cardData as Partial<CartaRelacionarColunas>).colunaB; } if (rest.tipo !== "PontoCerto") { delete (cardData as Partial<CartaPontoCerto>).imagemURL; delete (cardData as Partial<CartaPontoCerto>).zonasClicaveis; } if (rest.tipo !== "CompletarFrase") { delete (cardData as Partial<CartaCompletarFrase>).fraseIncompleta; delete (cardData as Partial<CartaCompletarFrase>).fragmentos; } if (!["Pergunta", "MultiplaEscolha", "Ordem", "Vantagem", "Desvantagem", "Outras", "ContraTempo"].includes(rest.tipo) || cardData.opcoes === undefined ) { delete cardData.opcoes; } else if (rest.tipo === "Ordem" && cardData.opcoes) { cardData.opcoes = cardData.opcoes.map(({ ordemTemp, ...o }) => o); } if (!cardData.baralho || cardData.baralho === DEFAULT_BARALHO_NAME) { delete cardData.baralho; } return cardData; }); };
    ...
    // --- JSX do Criador (Com indentação aprimorada) ---
    return (
        <div className="p-4 max-w-7xl mx-auto relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="flex space-x-4">
                            <Button onClick={() => downloadCode('js')} className="bg-blue-600 hover:bg-blue-700 text-white flex-1">
                                Baixar .js
                            </Button>
                            <Button onClick={() => downloadCode('json')} className="bg-purple-600 hover:bg-purple-700 text-white flex-1">
                                Baixar .json
                            </Button>
                        </div>
                         {cards.length > 0 && (
             ...
             {/* Seção Principal: Formulário e Preview */}
                    ...
                    <CardContent className="space-y-5 p-4 md:p-6">
                                 ...
                                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <Select value={dificuldade} onValueChange={(value) => setDificuldade(value as Dificuldade)}>
                                            <SelectTrigger id="card-dificuldade">
                                                <SelectValue placeholder="Selecione..." />
                                            </SelectTrigger>
                                            <SelectContent>
                                    ...
                                    <CardContent className="p-2 space-y-2">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <Input
                                                ...
                                                aria-label="URL da Imagem para Popup"
                                            />
                                            <Button
                                                type="button" size="sm" variant="outline"
                                                onClick={() => inserirTemplatePopup('imagem')}
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <Input
                                                ...
                                                className="text-xs h-8 flex-1"
                                                aria-label="URL do Vídeo MP4 para Popup"
                                            />
                                            <Button
                                                type="button" size="sm" variant="outline"
                                                onClick={() => inserirTemplatePopup('video')}
                                                className="h-8 text-xs px-2 shrink-0"
                                            >
                                                Add Vídeo Popup
                                            </Button>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <Input
                                                ...
                                                className="text-xs h-8 flex-1"
                                                aria-label="URL do YouTube para Popup"
                                            />
                                            <Button
                                                type="button" size="sm" variant="outline"
                                                onClick={inserirTemplatePopupYouTube}
                                                className="h-8 text-xs px-2 shrink-0"
                                            >
                                                Add YouTube Popup
                                            </Button>
                         ...
                         {["Pergunta", "MultiplaEscolha", "Ordem", "Vantagem", "Desvantagem", "Outras", "ContraTempo"].includes(tipo) && (
                            <Card className="border bg-gray-50 p-4 rounded-lg shadow-sm">
                                ...
                                <CardContent className="p-0 space-y-3">
                                                ...
                                                <li key={o.id} className="flex flex-col sm:flex-row sm:items-center sm:space-x-2 border-b pb-2 last:border-b-0">
                                                    ...
                                                    {tipo === "Ordem" && (
                                                        <div className="flex items-center space-x-1 my-1 sm:my-0">
                                                            ...
                                                            <Input
                                                                id={`order-${o.id}`} type="number" min="1" step="1"
                                                                onChange={(e) => handleSetOrder(o.id, e.target.value)}
                                                                value={o.ordemTemp ?? ""}
                                                                className="border p-1 w-16 rounded text-sm h-8 shrink-0"
                                                                aria-label={`Posição da opção ${o.id}`}
                                                            />
                                                        </div>
                                                    )}
                                                    {/* Botão Marcar Correta */}
                                                    {(tipo === "Pergunta" || tipo === "MultiplaEscolha" || tipo === "Outras" || tipo === "ContraTempo") && (
                                                        <Button
                                                            ...
                                                            size="sm"
                                                            className={cn("h-8 shrink-0 w-20", respostaCorreta.includes(o.id) && "bg-green-600 hover:bg-green-700")}
                                                        >
                                    ...
                                    {/* Textos de ajuda */}
                                    {tipo === "Ordem" && <p className="text-xs text-gray-500 mt-2">Defina a posição correta para cada opção (1, 2, 3...). Devem ser únicas.</p>}
                                    {tipo === "Vantagem" && <p className="text-xs text-green-600 mt-2">Para Vantagem, todas as opções são consideradas "corretas" (apenas para confirmação).</p>}
                                    {tipo === "Desvantagem" && <p className="text-xs text-red-600 mt-2">Para Desvantagem, não há resposta correta.</p>}
                                </CardContent>
                            </Card>
                        )}

                        {tipo === "ContraTempo" && (
                            <Card className="border bg-yellow-100 border-yellow-300 p-4 rounded-lg shadow-sm">
                                <CardContent className="p-0">
                                    <label htmlFor="tempo-limite" className="block text-sm font-medium mb-1 text-yellow-800">Tempo Limite (segundos)</label>
                                    <Input
                                        id="tempo-limite" type="number" value={tempoLimite}
                            ...
                            <Card className="border bg-blue-50 border-blue-200 p-4 rounded-lg shadow-sm">
                                ...
                                <CardContent className="p-0 grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <ScrollArea className="h-24 border rounded p-1 bg-white">
                                            <ul className="text-sm space-y-1">
                                                {colunaAItems.map(item => (
                                                    <li key={item.id} className="flex justify-between items-center px-1">
                                                        <span>{item.id}: {item.texto}</span>
                                                        <Button type="button" variant="ghost" className="text-red-500 h-6 w-6 p-0" onClick={() => handleRemoveColunaA(item.id)} aria-label={`Remover item A ${item.id}`}>X</Button>
                                                    </li>
                                                ))}
                                            </ul>
                                        </ScrollArea>
                                    ...
                                    <div>
                                        ...
                                        <ScrollArea className="h-24 border rounded p-1 bg-white">
                                            <ul className="text-sm space-y-1">
                                                {colunaBItems.map(item => (
                                                    <li key={item.id} className="flex justify-between items-center px-1">
                                                        <span>{item.id}: {item.texto}</span>
                                                        <Button type="button" variant="ghost" className="text-red-500 h-6 w-6 p-0" onClick={() => handleRemoveColunaB(item.id)} aria-label={`Remover item B ${item.id}`}>X</Button>
                                                    </li>
                                                ))}
                                            </ul>
                                        </ScrollArea>
                                    ...
                                    <div className="md:col-span-2">
                                        <label htmlFor="pares-corretos" className="block text-sm font-medium mb-1">Pares Corretos (Formato: IDa-IDb, IDa-IDb)</label>
                                        <Textarea
                                            id="pares-corretos" value={paresCorretosInput}
                        ...
                        {tipo === "PontoCerto" && (
                             <Card className="border bg-indigo-50 border-indigo-200 p-4 rounded-lg shadow-sm">
                                ...
                                <CardContent className="p-0 space-y-4">
                                    <div>
                                        <label htmlFor="imagem-url" className="block text-sm font-medium mb-1">URL da Imagem Principal*</label>
                                        <Input id="imagem-url" type="text" value={imagemURLPontoCerto} onChange={e => setImagemURLPontoCerto(e.target.value)} placeholder="/images/mapa.png" />
                                    </div>
                                    {imagemURLPontoCerto && (
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-2 border p-2 rounded items-end bg-white">
                                            <Input aria-label="Posição X" type="number" placeholder="X (0-1)" value={novaZona.x ?? ""} onChange={e => handleNovaZonaChange('x', e.target.value)} className="text-sm h-9" step="0.01" min="0" max="1"/>
                                            <Input aria-label="Posição Y" type="number" placeholder="Y (0-1)" value={novaZona.y ?? ""} onChange={e => handleNovaZonaChange('y', e.target.value)} className="text-sm h-9" step="0.01" min="0" max="1"/>
                                            <Input aria-label="Largura" type="number" placeholder="Largura (0-1)" value={novaZona.largura ?? ""} onChange={e => handleNovaZonaChange('largura', e.target.value)} className="text-sm h-9" step="0.01" min="0.01" max="1"/>
                                            <Input aria-label="Altura" type="number" placeholder="Altura (0-1)" value={novaZona.altura ?? ""} onChange={e => handleNovaZonaChange('altura', e.target.value)} className="text-sm h-9" step="0.01" min="0.01" max="1"/>
                                            <Input aria-label="Descrição da Zona" type="text" placeholder="Descrição (Opc)" value={novaZona.descricao ?? ""} onChange={e => handleNovaZonaChange('descricao', e.target.value)} className="text-sm h-9 col-span-2 sm:col-span-3"/>
                                            ...
                                            <ul className="text-sm space-y-1">
                                                {zonasClicaveis.map(z => (
                                                    <li key={z.id} className="flex justify-between items-center odd:bg-gray-50 even:bg-white px-1 py-0.5">
                                                        <span>ID:{z.id} ({z.x.toFixed(2)},{z.y.toFixed(2)} {z.largura.toFixed(2)}x{z.altura.toFixed(2)}) {z.descricao}</span>
                                                        <div className="flex items-center gap-1">
                                                            ...
                                                            <Button type="button" variant="ghost" className="text-red-500 h-6 w-6 p-0" onClick={() => handleRemoveZona(z.id)} aria-label={`Remover Zona ${z.id}`}>X</Button>
                                                        </div>
                                                    </li>
                                                ))}
                        ...
                        {tipo === "CompletarFrase" && (
                            <Card className="border bg-pink-50 border-pink-200 p-4 rounded-lg shadow-sm">
                                 ...
                                 <CardContent className="p-0 space-y-4">
                                    <div>
                                        <label htmlFor="frase-incompleta" className="block text-sm font-medium mb-1">Frase Incompleta* (use __1__, __2__...)</label>
                                        <Textarea id="frase-incompleta" value={fraseIncompleta} onChange={e => setFraseIncompleta(e.target.value)} className="h-20 font-mono text-sm" placeholder="O __1__ é essencial para a __2__."/>
                                    </div>
                                    <div>
                                        ...
                                        <div className="flex space-x-2 mb-2">
                                         ...
                                         <ScrollArea className="h-24 border rounded p-1 bg-white">
                                            <ul className="text-sm space-y-1">
                                                {fragmentos.map(f =>
                                                    <li key={f.id} className="flex justify-between items-center px-1">
                                                        <span>{f.id}: {f.texto}</span>
                                                        <Button type="button" variant="ghost" className="text-red-500 h-6 w-6 p-0" onClick={() => handleRemoveFragmento(f.id)} aria-label={`Remover fragmento ${f.id}`}>X</Button>
                                                    </li>
                                                )}
                                            </ul>
                                         </ScrollArea>
                                    ...
                                    <div>
                                         <label htmlFor="ordem-fragmentos" className="block text-sm font-medium mb-1">Ordem Correta* (IDs dos fragmentos, separados por vírgula)</label>
                                         <Input id="ordem-fragmentos" type="text" value={ordemFragmentos} onChange={e => setOrdemFragmentos(e.target.value)} placeholder="1, 3, 2" />
                                    </div>
                                </CardContent>
                            </Card>
                        )}
                                ...
                                <div>
                                    ...
                                    <ScrollArea className="h-24 border rounded p-1 bg-white">
                                        <ul className="space-y-1 text-sm">
                                            {categorias.map((c, i) =>
                                                <li key={i} className="flex justify-between items-center px-1">
                                                    <span>{c}</span>
                                                    <Button type="button" variant="ghost" className="text-red-500 h-6 w-6 p-0" onClick={() => handleRemoveCategoria(c)} aria-label={`Remover categoria ${c}`}>X</Button>
                                                </li>
                                            )}
                                        </ul>
                                    </ScrollArea>
                                    ...
                                    <ScrollArea className="h-24 border rounded p-1 bg-white">
                                        <ul className="space-y-1 text-sm">
                                            {fontes.map((f, i) =>
                                                <li key={i} className="flex justify-between items-center px-1">
                                                    <span>{f}</span>
                                                    <Button type="button" variant="ghost" className="text-red-500 h-6 w-6 p-0" onClick={() => handleRemoveFonte(f)} aria-label={`Remover fonte ${f}`}>X</Button>
                                                </li>
                                            )}
                                        </ul>
                                    </ScrollArea>
                        ...
                        <Card className="border bg-slate-50 p-4 rounded-lg shadow-sm">
                             <CardHeader className="p-0 mb-3">
                                <CardTitle className="text-lg">Feedback da Carta</CardTitle>
                            </CardHeader>
                             <CardContent className="p-0 space-y-3">
                                                ...
                                                <CardContent className="p-2 flex items-center justify-between gap-2">
                                                    <div className="flex-1 overflow-hidden">
                                                        <p className="text-xs font-semibold text-blue-700">
                                                            {c.tipo} - {c.dificuldade} {c.baralho && c.baralho !== DEFAULT_BARALHO_NAME ? `(${c.baralho})`: ''}
                                                        </p>
